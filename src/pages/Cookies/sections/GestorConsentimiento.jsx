                ...
                <div className="container mx-auto px-6 max-w-7xl relative z-20">
                    <div className="text-center mb-4 relative z-10">
                        <span className="inline-block py-1.5 px-4 rounded-full bg-[#FF4D00]/10 text-[#FF4D00] text-[10px] font-black tracking-widest uppercase mb-4 border border-[#FF4D00]/20">
                            <i className="fas fa-sliders text-[#FF4D00] mr-1"></i> Tu Panel de Control
                        </span>
                        <h2 className="text-3xl sm:text-4xl md:text-5xl font-black font-heading text-[#8B4513] mb-3 leading-tight">
                            Gestioná tus <span className="relative inline-block text-[#FF4D00]">
                                cookies
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
                            ...
                            <div className="bg-white rounded-[2rem] ring-1 ring-orange-100 shadow-xl shadow-orange-950/5 relative overflow-hidden h-full flex flex-col">
                                ...
                                <div className="relative z-10 p-6 flex flex-col flex-1">
                                    ...
                                    <div className="flex flex-col items-center text-center mb-5 pb-4 border-b border-dashed border-orange-100">
                                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#FF4D00] to-[#CC3D00] text-white flex items-center justify-center text-3xl shadow-lg shadow-orange-500/25 mb-3">
                                            <i className="fas fa-shield-halved"></i>
                                        </div>
                                        <h3 className="text-sm font-black font-heading text-[#8B4513]">Nivel de privacidad</h3>
                                        <p className="text-[10px] text-slate-500 font-semibold mt-1">{aceptadas} de {total} cookies activas</p>
                                    </div>

                                    {/* Porcentaje */}
                                    <div className="text-center mb-4">
                                        <span className="text-4xl font-black font-heading text-[#FF4D00]">{porcentaje}%</span>
                                    ...
                                    <div className="mb-4">
                                        <div className="h-3.5 rounded-full bg-orange-100 overflow-hidden border border-orange-50">
                                            <div className="h-full rounded-full bg-gradient-to-r from-[#FF4D00] to-[#8B4513] transition-all duration-700" style={{ width: `${porcentaje}%` }}></div>
                                        </div>
                                    </div>
                                    ...
                                    {/* Estado actual */}
                                    <div className="flex items-center gap-2.5 p-3 rounded-xl bg-[#FFF6F6] border border-orange-100 mb-4">
                                        <div className="w-9 h-9 rounded-lg bg-white border border-orange-100 text-[#FF4D00] flex items-center justify-center shrink-0">
                                    ...
                                    <div className="mt-auto pt-4 border-t border-dashed border-orange-100 grid grid-cols-2 gap-2.5">
                                        <button onClick={guardar} className="inline-flex items-center justify-center gap-2 py-3 rounded-full bg-[#FF4D00] hover:bg-[#CC3D00] text-white font-black text-[9px] uppercase tracking-[0.15em] shadow-lg shadow-orange-500/20 hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all">
                                            <i className="fas fa-check text-xs"></i> Guardar
                                        </button>
                                        <button onClick={() => setEstados({ esenciales: true, funcionales: false, analiticas: false, publicidad: false })} className="inline-flex items-center justify-center gap-2 py-3 rounded-full bg-[#5D3A1F] hover:bg-[#452A16] text-white font-black text-[9px] uppercase tracking-[0.15em] shadow-lg transition-all hover:-translate-y-0.5 active:scale-95">
                        ...
                        <div className="lg:col-span-7">
                            <div className="bg-white rounded-[2rem] ring-1 ring-orange-100 shadow-xl shadow-orange-950/5 relative overflow-hidden h-full">
                                ...
                                <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-[#5D3A1F]/10 blur-3xl animate-float-slow pointer-events-none"></div>

                                <div className="relative z-10 p-5 sm:p-6">
                                    <div className="flex items-center gap-3 mb-4 pb-3 border-b border-dashed border-orange-100">
                                        <div className="w-10 h-10 rounded-xl bg-[#FF4D00]/10 text-[#FF4D00] flex items-center justify-center shadow-sm">
                                            <i className="fas fa-cookie-bite text-lg"></i>
                                        </div>
                                        <div>
                                            <p className="text-sm font-black font-heading text-[#8B4513] leading-tight">Tipos de cookies</p>
                                            ...
                                            <div key={c.id} className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${estados[c.id] ? 'bg-[#FFFDF9] border-orange-100 shadow-sm' : 'bg-white border-orange-50'}`}>
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider ${c.obligatoria ? 'bg-[#FF4D00]/10 text-[#FF4D00] border border-[#FF4D00]/15' : c.soft}`}>
                                                ...
                                                <button
                                                    ...
                                                    className={`relative w-14 h-8 rounded-full transition-colors duration-300 shrink-0 ${estados[c.id] ? 'bg-[#FF4D00]' : 'bg-gray-300'} ${c.obligatoria ? 'cursor-not-allowed opacity-90' : 'cursor-pointer'}`}
