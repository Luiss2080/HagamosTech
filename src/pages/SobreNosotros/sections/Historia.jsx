...
const ProcesoTrabajo = () => {
    ...
    return (
        <section id="origen" className="pt-16 pb-12 relative overflow-hidden reveal scroll-mt-24 bg-[#050505] text-white border-t border-white/5">
            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="text-center mb-8 max-w-4xl mx-auto">
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-black font-heading text-white leading-tight">
                    ...
                    <p className="mt-4 text-slate-400 font-medium leading-relaxed max-w-2xl mx-auto text-base sm:text-lg">
                    ...
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 max-w-3xl mx-auto">
                        {[
                            ...
                            <div key={item.label} className="rounded-xl bg-[#111111] text-white border border-white/5 shadow-sm px-3 py-2.5 flex items-center justify-center gap-2">
                ...
                <div className="relative max-w-4xl mx-auto pb-6">
                    ...
                    {steps.map((step, index) => (
                        <div key={index} className="flex flex-col lg:flex-row items-center gap-5 lg:gap-12 mb-8 lg:mb-10 last:mb-0 group relative">
                            ...
                            <div className={`hidden lg:block absolute top-1/2 h-0.5 bg-white/10 z-0 ${index % 2 === 0 ? 'right-1/2 w-14 bg-gradient-to-l' : 'left-1/2 w-14 bg-gradient-to-r'} from-[#84CC16]/60 to-transparent`}></div>

                            <div className="hidden lg:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-[#050505] border-4 border-[#84CC16] z-10 shadow-sm group-hover:scale-125 transition-transform duration-300"></div>

                            <div className="order-1 lg:order-2 relative z-10 shrink-0">
                                <div className={`w-20 h-20 md:w-22 md:h-22 rounded-[1.6rem] ${step.bg} ${step.color} flex items-center justify-center text-3xl shadow-lg shadow-black/20 relative border-4 border-[#111111] ring-1 ring-white/5 group-hover:scale-105 transition-transform duration-500`}>
                                    <i className={`fa-solid ${step.icon}`}></i>
                                    <div className="absolute -top-2 -right-4 w-12 h-6 rounded-full bg-[#A3E635] text-[#0A0A0A] flex items-center justify-center text-[9px] font-black border-[3px] border-[#111111]">
                                        {step.num}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={`order-2 ${index % 2 === 0 ? 'lg:order-1 lg:text-right' : 'lg:order-3 lg:text-left'} flex-1 text-center lg:text-inherit`}>
                                <h3 className="text-xl md:text-2xl font-black font-heading text-white mb-2 group-hover:text-[#84CC16] transition-colors">{step.title}</h3>
