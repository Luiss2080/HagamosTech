...
const RETENCION = [
    { icon: 'fa-id-card', titulo: 'Datos de cuenta', detalle: 'Nombre, email y preferencias', duracion: 'Mientras tu cuenta esté activa', dias: 100, color: 'from-[#FF4D00] to-[#CC3D00]', nota: 'Se eliminan al cerrar tu cuenta.', items: ['Nombre y email', 'Preferencias', 'Configuración'] },
    { icon: 'fa-receipt', titulo: 'Historial de compras', detalle: 'Pedidos y facturas', duracion: '5 años (obligación fiscal)', dias: 75, color: 'from-[#5D3A1F] to-[#452A16]', nota: 'Por requisito legal de facturación.', items: ['Pedidos', 'Facturas', 'NIT'] },
    { icon: 'fa-envelope', titulo: 'Comunicaciones', detalle: 'Emails y WhatsApp', duracion: '2 años', dias: 50, color: 'from-[#8B4513] to-[#6B3410]', nota: 'Se purgan automáticamente.', items: ['Emails', 'Chats', 'Alertas'] },
    { icon: 'fa-cookie-bite', titulo: 'Cookies', detalle: 'Preferencias de navegación', duracion: '30 días', dias: 25, color: 'from-[#CC3D00] to-[#AA3000]', nota: 'Renovables con cada visita.', items: ['Preferencias', 'Sesión', 'Analítica'] },
];
                ...
                <div className="container mx-auto px-6 max-w-7xl relative z-20">
                    <div className="text-center mb-4 relative z-10">
                        <span className="inline-block py-1.5 px-4 rounded-full bg-[#5D3A1F]/10 text-[#5D3A1F] text-[10px] font-black tracking-widest uppercase mb-4 border border-[#5D3A1F]/20">
                            <i className="fas fa-hourglass-half text-[#5D3A1F] mr-1"></i> Retención de Datos
                        </span>
                        <h2 className="text-3xl sm:text-4xl md:text-5xl font-black font-heading text-[#8B4513] mb-3 leading-tight">
                            ¿Cuánto guardamos <span className="relative inline-block text-[#FF4D00]">
                                tus datos?
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`relative flex items-center gap-3 p-3.5 rounded-2xl text-left transition-all duration-300 ${
                                            isActive ? 'bg-white shadow-lg shadow-orange-950/10 ring-2 ring-[#FF4D00]/40 scale-[1.01]' : 'bg-white/60 border border-orange-50 hover:bg-white hover:shadow-md'
                                        }`}
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className={`font-black text-sm ${isActive ? 'text-[#FF4D00]' : 'text-[#111827]'}`}>{r.titulo}</p>
                                            <p className="text-[10px] text-slate-500 font-semibold truncate">{r.detalle}</p>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className={`text-sm font-black font-heading ${isActive ? 'text-[#FF4D00]' : 'text-[#8B4513]'}`}>{r.dias}%</p>
                        ...
                        <div className="lg:col-span-7 bg-white rounded-[2rem] ring-1 ring-orange-100 shadow-xl shadow-orange-950/5 relative overflow-hidden flex flex-col">
                            ...
                            <div className="absolute -bottom-12 -left-8 w-44 h-44 rounded-full bg-[#5D3A1F]/10 blur-3xl animate-float-medium pointer-events-none"></div>
                            ...
                            <div className="relative z-10 p-6 sm:p-8 flex flex-col flex-1">
                                <div className="flex items-center gap-4 mb-5 pb-4 border-b border-dashed border-orange-100">
                                    ...
                                    <span className="ml-auto inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#FFF6F6] text-[#8B4513] text-[9px] font-black uppercase tracking-widest border border-orange-100 shrink-0">
                                        <i className="fas fa-clock text-[#FF4D00]"></i>{actual.duracion}
                                    </span>
                                </div>
                                ...
                                <div className="mb-5">
                                    <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1.5">
                                        <span><i className="fas fa-hourglass-start text-[#FF4D00] mr-1"></i>Inicio</span>
                                        <span><i className="fas fa-hourglass-end text-[#FF4D00] mr-1"></i>Purgado</span>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                                    {actual.items.map((item, j) => (
                                        <span key={j} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#FFF6F6] border border-orange-100 text-[#8B4513] text-[10px] font-black uppercase tracking-wider">
                                            <i className="fas fa-circle-check text-[#FF4D00] text-[9px]"></i>{item}
                                        </span>
                                    ))}
                                </div>

                                <div className="mt-auto flex items-start gap-3 p-4 rounded-2xl bg-[#FFF6F6] border border-orange-100">
                                    <div className="w-8 h-8 rounded-lg bg-white border border-orange-100 text-[#FF4D00] flex items-center justify-center shrink-0 shadow-sm">
