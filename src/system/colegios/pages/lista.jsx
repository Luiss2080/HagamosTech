...
const ColegiosLista = ({
  colegiosFiltrados, paginatedColegios,
  onViewDetail, onEdit, onDelete,
  buscarColegio, setBuscarColegio,
  searchCriterion, setSearchCriterion,
  sortPreset, setSortPreset,
  sortDirection, setSortDirection,
  onSearch, onNavigateToClientes
}) => {
  ...
  const criterioOptions = [
    ...
    { value: 'nombre', label: 'Nombre Colegio', icon: 'fas fa-font' },
  ];
  ...
  const smartResults = useMemo(() => {
    return colegiosFiltrados.slice(0, 5);
  }, [colegiosFiltrados]);

  const handleSelectSmartItem = (item) => {
    setBuscarColegio(item.nombre);
    setShowSmartResults(false);
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-25">
        <div className="bg-gradient-to-r from-orange-50/80 to-transparent dark:from-[#E95A0C]/10 dark:to-transparent p-4 rounded-xl border border-orange-200/50 dark:border-[#E95A0C]/20 mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-505 font-bold uppercase tracking-widest leading-normal m-0 mt-0.5">
              Búsqueda avanzada de colegios por múltiples criterios de búsqueda.
            </p>
          </div>
        </div>
        <style>{`
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
          ...
          <div className="sm:col-span-5 space-y-1 relative" ref={searchContainerRef}>
            <label className="block text-[9px] font-black uppercase tracking-wider text-[#E95A0C]">BUSCAR COLEGIO</label>
            <div className="relative">
              <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              <input type="text" placeholder="Buscar colegio de forma inteligente..." value={buscarColegio}
                onFocus={() => setShowSmartResults(true)}
                onChange={(e) => { setBuscarColegio(e.target.value); setShowSmartResults(true); }}
                  ...
                  smartResults.map((item, index) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <i className="fas fa-school text-[#E95A0C] text-[10px]"></i>
        ...
        <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-100">
          ...
          <button onClick={onNavigateToClientes}
            className="px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer bg-slate-100 dark:bg-[#070710] text-[#E95A0C] hover: flex items-center gap-2 border border-orange-200/50 dark:dark:border-orange-900/50">
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-[#4A2E1B] dark:text-white uppercase tracking-wider m-0 leading-tight flex items-center gap-2">
                LISTADO DE COLEGIO / INSTITUCIÓN
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest m-0 mt-0.5">
                ...
                Fichas de colegios y convenios de capacitación
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className=" text-[#E95A0C] border border-orange-200/50 dark:dark:border-orange-900/50 text-[10px] px-4 py-1.5 rounded-xl font-black uppercase shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs">{colegiosFiltrados.length}</span>
        ...
        <div className="w-full overflow-x-auto no-scrollbar">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-100 dark:border-white/5 dark:border-white/5 text-[#E95A0C] uppercase text-[9px] font-black tracking-widest                 <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-school text-[#E95A0C] mr-1.5"></i>COLEGIO / JURÍDICO</th>
                ...
                <th className="py-3.5 px-4 text-center w-32 whitespace-nowrap"><i className="fas fa-calendar text-[#E95A0C] mr-1.5"></i>REGISTRO</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {paginatedColegios.length > 0 ? (
                paginatedColegios.map((c, idx) => (
                  <tr key={idx} className="hover: dark:hover:bg-red-950/20 transition-colors" style={{ transitionDelay: `${idx * 60}ms` }}>
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-[#E95A0C] font-mono font-bold block mt-0.5">{c.parsed?.correo || 'correo@colegio.edu'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700 dark:text-slate-300 text-xs">{c.parsed?.telefono || 'S/T'}</td>
                    <td className="py-3.5 px-4">
                      <span className="text-slate-600 dark:text-slate-400 text-xs block truncate max-w-[180px] font-medium">{c.parsed?.direccion || 'Sin dirección'}</span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase inline-flex items-center gap-1.5 shadow-sm ${
                        c.parsed?.estado === 'Inactivo'
                          ? 'bg-rose-100 dark:bg-rose-950/40 text-rose-800 dark:text-red-400'
                          : 'bg-emerald-100 dark:bg-emerald-950/40 text-[#E95A0C]merald-800 dark:text-[#E95A0C]merald-400'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/400 ${c.parsed?.estado === 'Inactivo' ? 'bg-rose-50 dark:bg-rose-950/400' : 'bg-emerald-50 dark:bg-emerald-950/400 animate-pulse'}`}></span> 
                        {c.parsed?.estado || 'ACTIVO'}
                      </span>
                    </td>
                    <td className="py-3.5 px-4">
                ...
                <tr>
                  <td colSpan="6" className="py-12 text-center text-slate-400 font-bold uppercase tracking-wider">
                    <i className="fas fa-school text-3xl text-[#E95A0C] block mb-3.5"></i>
                    Aún no hay colegios registrados bajo esta categoría.
                  </td>
                </tr>
              )}
            </tbody>
