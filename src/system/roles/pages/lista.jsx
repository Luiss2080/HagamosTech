...
const RolesLista = ({
  ...
  const criterioOptions = [
    ...
    { value: 'nombre', label: 'Nombre del Rol', icon: 'fas fa-font' },
  ];
  ...
  const smartResults = useMemo(() => {
    return rolesFiltrados.slice(0, 5);
  }, [rolesFiltrados]);

  const handleSelectSmartItem = (item) => {
    setBuscarRol(item.nombre);
    setShowSmartResults(false);
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-25">
        <div className="bg-gradient-to-r from-orange-50/80 to-transparent dark:from-[#E95A0C]/10 dark:to-transparent p-4 rounded-xl border border-orange-200/50 dark:border-[#E95A0C]/20 mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest leading-normal m-0 mt-0.5">
              Búsqueda avanzada de roles por múltiples criterios y ordenación.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
          ...
          <div className="sm:col-span-5 space-y-1 relative" ref={searchContainerRef}>
            <label className="block text-[9px] font-black uppercase tracking-wider text-[#E95A0C]">BUSCAR ROL</label>
            <div className="relative">
              <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              <input type="text" placeholder="Buscar rol de forma inteligente..." value={buscarRol}
                onFocus={() => setShowSmartResults(true)}
                onChange={(e) => { setBuscarRol(e.target.value); setShowSmartResults(true); }}
                  ...
                  smartResults.map((item, index) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <i className="fas fa-user-tag text-[#E95A0C] text-[10px]"></i>
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest m-0 mt-0.5">
                ...
                Gestión de niveles de acceso y perfiles de seguridad
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className=" text-[#E95A0C] border border-orange-200/50 dark:dark:border-orange-900/50 text-[10px] px-4 py-1.5 rounded-xl font-black uppercase shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs">{rolesFiltrados.length}</span>
              ROLES EN EL SISTEMA
            </span>
          </div>
        </div>
        ...
        {paginatedRoles.length > 0 ? (
              ...
              <div key={idx} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 dark:bg-[#070710] border border-slate-100 hover:border-[#E95A0C]/30 hover:shadow-md transition-all duration-200 group">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <span className="font-extrabold text-[#4A2E1B] dark:text-white uppercase tracking-wider text-xs block leading-tight truncate">
                      {r.nombre}
                    </span>
                    <span className="text-[9px] font-mono font-bold text-slate-400 mt-0.5 block">
                      <i className="fas fa-hashtag text-[8px] mr-0.5"></i>{r.id}
                      {r.created_at && (
                        <span className="ml-2 font-sans">
                          <i className="fas fa-calendar text-[7px] mr-0.5"></i>
                          {new Date(r.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1 shrink-0 ml-2">
                  ...
                  <button type="button" onClick={() => onEdit(r)}
                    disabled={r.id === 1}
                    className="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-400 to-amber-600 hover:from-amber-500 hover:to-amber-700 text-white flex items-center justify-center text-[10px] shadow-md shadow-amber-500/20 dark:shadow-none transition-all cursor-pointer border-0 hover:scale-110 disabled:opacity-30 disabled:pointer-events-none active:scale-95"
                    title="Editar Rol">
                    <i className="fas fa-pen"></i>
                  </button>
                  <button type="button" onClick={() => onDelete(r)}
                    disabled={r.id === 1 || r.id === 4}
                    className="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 text-white flex items-center justify-center text-[10px] shadow-md shadow-rose-500/20 transition-all cursor-pointer border-0 hover:scale-110 disabled:opacity-30 disabled:pointer-events-none active:scale-95"
                    title="Eliminar Rol">
          ...
          <div className="py-12 text-center font-bold text-slate-400">
            <i className="fas fa-info-circle text-3xl text-[#E95A0C] block mb-2 opacity-50 animate-pulse"></i>
