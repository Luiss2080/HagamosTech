...
const PermisoEditView = ({
  ...
  const [modulo, setModulo] = useState(
    permisoForm.nombre?.toLowerCase().includes('venta') ? 'Ventas' : 
    permisoForm.nombre?.toLowerCase().includes('stock') || permisoForm.nombre?.toLowerCase().includes('inventario') ? 'Almacenes' : 'Seguridad'
  );
  ...
  const deptoOptions = [
    ...
    { value: 'Seguridad', label: 'Seguridad y Accesos', icon: 'fas fa-user-shield' },
  ];

  const getPermisoNotes = () => {
    if (permisoForm.nombre?.toLowerCase().includes('venta')) return 'Habilita operaciones comerciales como cotización, facturación o anulación de ventas.';
    if (permisoForm.nombre?.toLowerCase().includes('stock') || permisoForm.nombre?.toLowerCase().includes('inventario')) return 'Controla permisos del almacén como el ingreso de stock o la visualización del historial de inventario.';
    return 'Controla permisos relacionados a accesos de seguridad, administración de usuarios y asignaciones generales.';
  };
  ...
  return (
    <div className="space-y-5 font-montserrat w-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">
              ...
              <span>Permisos</span>
              <span>{'>'}</span>
              <span className="text-[#E95A0C]">Editar Permiso</span>
            </div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mt-1 m-0">
              Modifique el código identificador y alcance del permiso seleccionado en el sistema.
            </p>
          </div>
        </div>
      ...
      <form onSubmit={submitPermiso} className="space-y-5">
        ...
        <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-6 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C]">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-150 mb-4">
            ...
            <h4 className="text-xs font-black text-[#4A2E1B] dark:text-white uppercase tracking-wider m-0">
              DATOS DE LA LLAVE DE SEGURIDAD #{permisoForm.id}
            </h4>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1 md:col-span-2">
              <label className="block text-[10px] font-black uppercase tracking-wider text-[#E95A0C]">
                CÓDIGO / IDENTIFICADOR DEL PERMISO <b className="text-red-500">*</b>
              </label>
              <div className="relative">
                <i className="fas fa-key absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                <input
                  type="text"
                  placeholder="Ej: CATALOGO_CREAR, STOCK_SALIDA, etc."
                  required
                  value={permisoForm.nombre}
                  onChange={(e) => setPermisoForm({ id: permisoForm.id, nombre: e.target.value.toUpperCase() })}
                  className="w-full pl-9 pr-3.5 py-2.5 rounded-xl border border-slate-200/60 dark:border-white/5 bg-slate-50 dark:bg-[#070710] text-slate-800 dark:text-white dark:border-white/8 dark:text-slate-200 text-xs focus:ring-2 focus:ring-[#E95A0C]/40 outline-none font-mono font-bold"
                />
              </div>
            </div>
            ...
            <div className="space-y-1 md:col-span-2">
              <label className="block text-[10px] font-black uppercase tracking-wider text-[#E95A0C]">
                MÓDULO / DEPARTAMENTO DEL PERMISO
              </label>
              <DropdownSelect value={modulo} onChange={setModulo} options={deptoOptions} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-6 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C]">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <textarea
                rows="3"
                defaultValue={getPermisoNotes()}
                placeholder="Ej: Habilita el botón de facturación o la capacidad de anular cotizaciones..."
            ...
            <div className="bg-amber-50 dark:bg-amber-950/40 p-4 rounded-xl border border-amber-200 flex items-start gap-3">
              ...
              <div className="text-xs text-[#E95A0C]mber-700 font-medium">
                ...
                Cualquier cambio realizado en el código identificador se propagará inmediatamente a todas las asignaciones existentes de la matriz de accesos y a los roles que lo utilicen.
              </div>
            </div>
...
export default PermisoEditView;
