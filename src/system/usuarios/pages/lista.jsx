  ...
  return (
    <div className="relative w-full" ref={ref}>
      <button type="button" onClick={() => setOpen(!open)}
        className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#040408] text-[#6B442A] dark:text-white dark:border-white/8 dark:text-slate-200 text-xs font-bold flex items-center justify-between gap-2 focus:ring-2 focus:ring-[#E95A0C]/40 outline-none cursor-pointer transition-all hover:border-[#E95A0C]/40">
          ...
          {options.map(opt => (
            <button key={opt.value} type="button"
              ...
              className={`w-full px-3.5 py-2.5 text-xs font-bold flex items-center gap-2 text-left transition-all cursor-pointer ${
                opt.value === value ? 'bg-[#8B4513] text-white' : 'text-[#6B442A] dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
               }`}>
...
const UsuariosLista = ({
  ...
  const criterioOptions = [
    ...
    { value: 'nombre', label: 'Nombre', icon: 'fas fa-font' },
    { value: 'correo', label: 'Correo', icon: 'fas fa-envelope' },
  ];

  const sortOptions = [
    { value: 'recientes', label: 'Fecha Registro', icon: 'fas fa-calendar' },
    { value: 'nombre_az', label: 'Nombre A-Z', icon: 'fas fa-arrow-down-a-z' },
  ];
  ...
  const handleSelectSmartItem = (item) => {
    setUserSearch(`${item.nombre} ${item.apellido}`);
    setShowSmartResults(false);
  ...
  return (
    <div className="space-y-4 font-montserrat w-full">
      {/* SECCIÓN FILTROS Y BÚSQUEDA GLOBAL DE USUARIOS */}
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-25">
        <div className="bg-gradient-to-r from-orange-50/80 to-transparent dark:from-[#E95A0C]/10 dark:to-transparent p-4 rounded-xl border border-orange-200/50 dark:border-[#E95A0C]/20 mb-4 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-[#4A2E1B] dark:text-white uppercase tracking-wider m-0">SEGURIDAD Y CONTROL</h3>
            <p className="text-[10px] text-slate-505 font-bold uppercase tracking-widest leading-normal m-0 mt-0.5">
              Consulte y administre las cuentas de usuarios activos y configure los perfiles de acceso.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
          ...
          <div className="sm:col-span-5 space-y-1 relative" ref={searchContainerRef}>
            <label className="block text-[9px] font-black uppercase tracking-wider text-[#E95A0C]">FILTRAR USUARIOS</label>
            <div className="relative">
              <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              <input type="text" placeholder="Buscar por nombre, usuario, correo o rol de forma inteligente..." value={userSearch}
                onFocus={() => setShowSmartResults(true)}
                onChange={(e) => { setUserSearch(e.target.value); setShowSmartResults(true); }}
                className="w-full pl-9 pr-3.5 py-2.5 rounded-xl border border-slate-200/60 dark:border-white/5 bg-slate-50 dark:bg-[#070710] text-[#6B442A] dark:text-white dark:border-white/8 dark:text-slate-200 text-xs focus:ring-2 focus:ring-[#E95A0C]/40 outline-none font-bold" />
            </div>
                ...
                {smartResults.length > 0 ? (
                  smartResults.map((item, index) => (
                    <button
                      ...
                      className="w-full px-3.5 py-2.5 text-xs font-bold flex items-center justify-between text-left transition-all cursor-pointer text-[#6B442A] dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 border-b border-slate-100/50 last:border-b-0"
                    >
                      <div className="flex items-center gap-2.5">
                        <i className="fas fa-user-gear text-[#E95A0C] text-[10px]"></i>
                        <span className="truncate uppercase font-extrabold">{item.nombre} {item.apellido}</span>
                      </div>
      ...
      {/* PANEL GENERAL DE USUARIOS (FULL WIDTH 100%) */}
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-10 flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b-2 border-slate-100 dark:border-white/5 dark:border-white/5">
          ...
          <span className=" text-[#E95A0C] border border-orange-200/50 dark:dark:border-orange-900/50 text-[10px] px-4 py-1.5 rounded-xl font-black uppercase shadow-sm flex items-center gap-2">
            ...
            <span className="text-xs">{usuariosFiltrados.length}</span>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-100 dark:border-white/5 dark:border-white/5 text-[#E95A0C] uppercase text-[9px] font-black tracking-widest">
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-user-tie text-[#E95A0C] mr-1.5"></i>USUARIO / NOMBRE</th>
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-envelope text-[#E95A0C] mr-1.5"></i>CORREO</th>
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-shield-halved text-[#E95A0C] mr-1.5"></i>ROL</th>
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-phone text-[#E95A0C] mr-1.5"></i>TELÉFONO</th>
                <th className="py-3.5 px-4 text-center w-28 whitespace-nowrap"><i className="fas fa-bolt text-[#E95A0C] mr-1.5"></i>ESTADO</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {paginatedUsers.length > 0 ? (
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl  text-[#E95A0C] flex items-center justify-center text-sm shrink-0 border border-orange-500/20 dark:border-orange-500/10 shadow-sm">
                          <i className={u.rolNombre === 'Administrador' ? "fas fa-user-shield" : "fas fa-user-gear"}></i>
                        </div>
                        <div>
                          <span className="font-extrabold text-[#4A2E1B] dark:text-white text-xs block leading-tight uppercase">
                            {u.nombre} {u.apellido}
                          </span>
                          <span className="text-[10px] text-[#E95A0C] font-mono font-bold block mt-0.5">
                            @{u.usuario || 'sin-usuario'}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-semibold text-[#6B442A] dark:text-slate-300 text-xs font-mono">{u.correo}</td>
                    <td className="py-3.5 px-4">
                      <span className="px-2.5 py-1 rounded-lg  text-[#E95A0C] text-[9px] font-black uppercase border border-orange-200/50 dark:dark:border-orange-900/50/60 shadow-sm">
                        {u.rolNombre}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 font-bold text-[#6B442A] dark:text-slate-300 text-xs">{u.numtel || <span className="text-slate-400 font-normal italic">N/A</span>}</td>
                    <td className="py-3.5 px-4 text-center">
                      <button
                        onClick={() => cambiarEstadoUsuario(u.id, u.activo)}
                        className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all cursor-pointer inline-flex items-center gap-1.5 shadow-sm border-0 ${
                          u.activo 
                            ? 'bg-emerald-100 dark:bg-emerald-950/40 text-[#E95A0C]merald-800 hover:bg-emerald-250 dark:text-[#E95A0C]merald-455' 
                            : 'bg-rose-100 dark:bg-rose-950/40 text-rose-800 hover:bg-rose-250 dark:text-red-400'
                        }`}
                      >
                ...
                <tr>
                  <td colSpan="6" className="py-12 text-center text-slate-400 font-bold uppercase">
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 font-montserrat">
          <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-6 max-w-sm w-full border border-slate-200/60 dark:border-white/5 shadow-2xl dark:shadow-black/80 relative animate-fade-in">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 mb-4">
              <h4 className="text-xs font-black text-slate-855 uppercase tracking-wider flex items-center gap-2 m-0">
            ...
            <div className="flex items-center gap-2.5 mt-6 pt-4 border-t border-slate-100">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-slate-100 dark:bg-[#070710] hover:bg-slate-200 dark:hover:bg-slate-750 text-[#6B442A] dark:text-slate-300 text-xs font-black uppercase tracking-wider transition-colors cursor-pointer border-0"
              >
                Limpiar
              </button>
              <button
