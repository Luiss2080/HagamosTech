...
const ReportesComprasAnalitica = ({ setActiveTab }) => {
  ...
  const generarReporte = async () => {
    ...
    try {
      const { data } = await apiClient.get(`/reportes-sistema/compras`);
      if (data.success) {
        setKpis(data.kpis);
    ...
    <div className="space-y-5 font-montserrat w-full">
      <BreadcrumbHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Reportes' },
          { label: 'Analítica de Compras' }
        ]}
        title="Dashboard Analítico de Compras"
        subtitle="Control de egresos, inventario adquirido y relación con proveedores."
...
export default ReportesComprasAnalitica;
