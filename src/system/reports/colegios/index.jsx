...
const ReportesColegiosAnalitica = ({ setActiveTab }) => {
  ...
  const generarReporte = async () => {
    ...
    try {
      const { data } = await apiClient.get(`/reportes-sistema/colegios`);
      if (data.success) {
        setKpis(data.kpis);
    ...
    <div className="space-y-5 font-montserrat w-full">
      <BreadcrumbHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Reportes' },
          { label: 'Analítica de Colegios' }
        ]}
        title="Dashboard Analítico de Colegios"
        subtitle="Análisis de penetración institucional, convenios y desempeño B2B."
...
export default ReportesColegiosAnalitica;
