...
const ReportesClientesAnalitica = ({ setActiveTab }) => {
  ...
  const generarReporte = async () => {
    ...
    try {
      const { data } = await apiClient.get(`/reportes-sistema/clientes`);
      if (data.success) {
        setKpis(data.kpis);
    ...
    <div className="space-y-5 font-montserrat w-full">
      <BreadcrumbHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Reportes' },
          { label: 'Analítica de Clientes' }
        ]}
        title="Dashboard Analítico de Clientes"
        subtitle="Métricas de retención, segmentación y crecimiento de la base de clientes naturales."
...
export default ReportesClientesAnalitica;
