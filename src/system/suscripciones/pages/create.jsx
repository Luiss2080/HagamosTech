  ...
  return (
    <div className="space-y-5 font-montserrat w-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">
              ...
              <span>{'>'}</span>
              <span className="text-[#E95A0C]">Nueva Suscripción</span>
            </div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mt-1 m-0">
              Cree un plan o apruebe un pago en efectivo para otorgar acceso al usuario.
            </p>
          </div>
        </div>
      ...
      <form onSubmit={submitSuscripcion} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-[10px] font-black uppercase tracking-wider text-[#E95A0C] mb-1.5">
                USUARIO A SUSCRIBIR: <b className="text-red-500">*</b>
              </label>
              {usuarioOptions.length > 0 ? (
                <DropdownSelect 
                  value={suscripcionForm.usuarioId || usuarioOptions[0].value}
                  onChange={(val) => setSuscripcionForm(prev => ({ ...prev, usuarioId: val }))}
                  options={usuarioOptions} 
                />
              ) : (
                <div className="text-xs font-bold text-slate-500 dark:text-slate-400">No hay usuarios disponibles.</div>
              )}
            </div>
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-wider text-[#E95A0C] mb-1.5">
                DÍAS DE PRUEBA O VIGENCIA: <b className="text-red-500">*</b>
              </label>
              <input type="number" required min="1" max="3650"
                placeholder="Ej: 365"
                value={suscripcionForm.diasPrueba || (suscripcionForm.estado === 'suscrito' ? 365 : 3)}
                onChange={(e) => setSuscripcionForm(prev => ({ ...prev, diasPrueba: e.target.value }))}
        ...
        {/* SECCION 2: PAGO Y REGISTRO (OPCIONAL SI ES INVITADO, NECESARIO SI ES PAGO EN EFECTIVO) */}
        <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-6 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C]">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-150 mb-4">
            <i className="fas fa-money-bill-wave text-[#E95A0C] text-sm"></i>
            <h4 className="text-xs font-black text-[#4A2E1B] dark:text-white uppercase tracking-wider m-0">
              REGISTRO DE PAGO (PARA SUSCRIPCIONES MANUALES)
            </h4>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea rows="3" placeholder="Ingrese detalles del pago, número de recibo, caja, o comprobante manual..."
...
export default SuscripcionCreateView;
