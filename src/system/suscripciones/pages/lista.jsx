...
const SUSCRIPCIONES_MOCK = [
  { id: 1, nombre: 'MARTA ADORNO', correo: 'marta.adorno@email.com', fechaInicio: '2026-06-01T10:00:00', fechaFinPrueba: '2026-09-01T10:00:00', estado: 'suscrito' },
  { id: 2, nombre: 'ADRIEL ALESSANDRO LINO MENDOZA', correo: 'adriel.lino@email.com', fechaInicio: '2026-06-15T14:30:00', fechaFinPrueba: '2026-09-15T14:30:00', estado: 'suscrito' },
  { id: 3, nombre: 'MIGUEL ANGEL PINTO ARAMAYO', correo: 'miguel.pinto@email.com', fechaInicio: '2026-05-10T09:00:00', fechaFinPrueba: '2026-08-10T09:00:00', estado: 'suscrito' },
  { id: 4, nombre: 'JOSE DAVID COLQUE BERNABE', correo: 'jose.colque@email.com', fechaInicio: '2026-07-01T11:15:00', fechaFinPrueba: '2026-10-01T11:15:00', estado: 'suscrito' },
  { id: 5, nombre: 'MAURICIO VASQUEZ MENACHO', correo: 'mauricio.vasquez@email.com', fechaInicio: '2026-04-20T08:45:00', fechaFinPrueba: '2026-05-20T08:45:00', estado: 'vencido' },
  { id: 6, nombre: 'ROBERTO VILLARROEL', correo: 'roberto.villarroel@email.com', fechaInicio: '2026-07-10T16:00:00', fechaFinPrueba: '2026-10-10T16:00:00', estado: 'invitado' },
  { id: 7, nombre: 'MARÍA GONZÁLEZ LOPEZ', correo: 'maria.gonzalez@email.com', fechaInicio: '2026-06-20T13:20:00', fechaFinPrueba: '2026-09-20T13:20:00', estado: 'suscrito' },
  { id: 8, nombre: 'LUIS MARIO PÉREZ', correo: 'luis.perez@email.com', fechaInicio: '2026-05-05T10:30:00', fechaFinPrueba: '2026-06-05T10:30:00', estado: 'vencido' },
  { id: 9, nombre: 'CARLA MENDOZA RIVERA', correo: 'carla.mendoza@email.com', fechaInicio: '2026-07-15T09:00:00', fechaFinPrueba: '2026-10-15T09:00:00', estado: 'invitado' },
  { id: 10, nombre: 'PEDRO SUAREZ VELASCO', correo: 'pedro.suarez@email.com', fechaInicio: '2026-07-12T15:45:00', fechaFinPrueba: '2026-10-12T15:45:00', estado: 'suscrito' },
];
...
const SuscripcionsLista = ({
  suscripcionesFiltradas: suscripcionesFiltradasProp = [],
  paginatedSuscripciones: paginatedSuscripcionesProp = [],
  onViewDetail, onEdit, onDelete,
  buscarSuscripcion, setBuscarSuscripcion,
  searchCriterion, setSearchCriterion,
  sortPreset, setSortPreset,
  sortDirection, setSortDirection,
  onSearch
}) => {
  const suscripcionesFiltradas = suscripcionesFiltradasProp.length > 0 ? suscripcionesFiltradasProp : SUSCRIPCIONES_MOCK;
  const paginatedSuscripciones = paginatedSuscripcionesProp.length > 0 ? paginatedSuscripcionesProp : SUSCRIPCIONES_MOCK;
  ...
  const criterioOptions = [
    ...
    { value: 'nombre', label: 'Nombre del Usuario', icon: 'fas fa-font' },
    { value: 'correo', label: 'Correo Electrónico', icon: 'fas fa-envelope' },
    { value: 'estado', label: 'Estado', icon: 'fas fa-bolt' }
  ];
  ...
  const smartResults = useMemo(() => {
    return suscripcionesFiltradas.slice(0, 5);
  }, [suscripcionesFiltradas]);

  const handleSelectSmartItem = (item) => {
    setBuscarSuscripcion(item.nombre);
    setShowSmartResults(false);
    ...
    if (s === 'suscrito') {
      return (
        <span className="px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950/40 text-[#E95A0C]merald-800 text-[9px] font-black uppercase inline-flex items-center gap-1.5 shadow-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span> SUSCRITO
        </span>
      );
    }
    if (s === 'invitado') {
      return (
        <span className="px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-950/40 text-[#E95A0C]mber-800 text-[9px] font-black uppercase inline-flex items-center gap-1.5 shadow-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-50 dark:bg-amber-950/400 animate-pulse"></span> INVITADO
        </span>
      );
    }
    return (
      <span className="px-3 py-1 rounded-full bg-rose-100 dark:bg-rose-950/40 text-rose-800 text-[9px] font-black uppercase inline-flex items-center gap-1.5 shadow-sm">
        <i className="fas fa-ban text-[8px]"></i> VENCIDO
      </span>
    );
  };
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-25">
        <div className="bg-gradient-to-r from-orange-50/80 to-transparent dark:from-[#E95A0C]/10 dark:to-transparent p-4 rounded-xl border border-orange-200/50 dark:border-[#E95A0C]/20 mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest leading-normal m-0 mt-0.5">
              Búsqueda avanzada de suscripciones por múltiples campos con paginación integrada.
            </p>
          </div>
        </div>
        <style>{`
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
          ...
          <div className="sm:col-span-5 space-y-1 relative" ref={searchContainerRef}>
            <label className="block text-[9px] font-black uppercase tracking-wider text-[#E95A0C]">BUSCAR SUSCRIPCIÓN</label>
            <div className="relative">
              <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              <input type="text" placeholder="Buscar suscripción de forma inteligente..." value={buscarSuscripcion}
                onFocus={() => setShowSmartResults(true)}
                onChange={(e) => { setBuscarSuscripcion(e.target.value); setShowSmartResults(true); }}
                  ...
                  smartResults.map((item, index) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <i className={"fas fa-user text-[#E95A0C] text-[10px]"}></i>
      ...
      <div className="bg-white dark:bg-[#040408] dark:border-white/8 rounded-2xl p-5 shadow-xl dark:shadow-black/60 border border-slate-200/60 dark:border-white/5 border-l-4 border-l-[#E95A0C] overflow-visible relative z-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5 pb-4 border-b-2 border-slate-100 dark:border-white/5 dark:border-white/5">
          ...
          <div className="flex items-center gap-3">
            <span className=" text-[#E95A0C] border border-orange-200/50 dark:dark:border-orange-900/50 text-[10px] px-4 py-1.5 rounded-xl font-black uppercase shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs">{suscripcionesFiltradas.length}</span>
        ...
        <div className="w-full overflow-x-auto no-scrollbar">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-100 dark:border-white/5 dark:border-white/5 text-[#E95A0C] uppercase text-[9px] font-black tracking-widest                 <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-user text-[#E95A0C] mr-1.5"></i>USUARIO</th>
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-calendar-check text-[#E95A0C] mr-1.5"></i>FECHA INICIO</th>
                <th className="py-3.5 px-4 whitespace-nowrap"><i className="fas fa-clock text-[#E95A0C] mr-1.5"></i>FIN PRUEBA</th>
                <th className="py-3.5 px-4 text-center whitespace-nowrap"><i className="fas fa-bolt text-[#E95A0C] mr-1.5"></i>ESTADO</th>
                <th className="py-3.5 px-4 text-center whitespace-nowrap"><i className="fas fa-sliders text-[#E95A0C] mr-1.5"></i>ACCIONES</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {paginatedSuscripciones.length > 0 ? (
                paginatedSuscripciones.map((c, idx) => (
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-[#E95A0C] font-mono font-bold block mt-0.5">{c.correo}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700 dark:text-slate-300 text-xs">{formatDate(c.fechaInicio)}</td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700 dark:text-slate-300 text-xs">{formatDate(c.fechaFinPrueba)}</td>
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center justify-center gap-1.5">
                        <button onClick={() => onViewDetail(c)}
                          ...
                          <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-[#8B4513] text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg dark:shadow-black/60 pointer-events-none z-10">Ver Detalles</span>
                        </button>
                        ...
                        <button onClick={() => onDelete(c)}
                          ...
                          title="Vencer Suscripción">
                          <i className="fas fa-ban"></i>
                          <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-[#8B4513] text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg dark:shadow-black/60 pointer-events-none z-10">Vencer</span>
                        </button>
                      </div>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan="5" className="text-center py-12 text-slate-450 font-bold">
...
export default SuscripcionsLista;
