        ...
        <div className="container mx-auto px-6 max-w-7xl relative z-20">
          <div className="text-center mb-4 relative z-10">
            <span className="inline-block py-1.5 px-4 rounded-full bg-[#5D3A1F]/10 text-[#5D3A1F] text-[10px] font-black tracking-widest uppercase mb-4 border border-[#5D3A1F]/20">
              <i className="fas fa-medal text-[#5D3A1F] mr-1"></i> Te lo Garantizamos
            </span>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black font-heading text-[#8B4513] mb-3 leading-tight">
              Comprá con <span className="relative inline-block text-[#FF4D00]">
                confianza
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {garantias.map((g, i) => (
              <div key={i} className={`relative rounded-[2rem] p-6 transition-all duration-500 group overflow-hidden border hover:-translate-y-1.5 z-10 text-center ${g.color} text-white shadow-xl shadow-orange-950/10`}>
                ...
                <div className="relative z-10">
                  ...
                  <h4 className="text-lg font-black mb-2 leading-tight">{g.titulo}</h4>
                  <p className="text-xs font-medium opacity-90 leading-relaxed">{g.desc}</p>
...
export default BeneficiosDetalle;
