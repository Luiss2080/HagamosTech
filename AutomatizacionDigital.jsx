import RecorridoIntegraciones from '../components/RecorridoIntegraciones';
...
const CLIENTES = ['Rintisa', 'La Calera', 'Tai Loy', 'Auna', 'BSF Almacenes', 'Grupo EFE'];
...
const PROCESO = [
  { title: 'Análisis de Datos', desc: 'Identificamos las fuentes de información y definimos el entrenamiento del modelo de lenguaje.' },
  { title: 'Estructuración de Prompts', desc: 'Diseñamos directrices avanzadas (System Prompts) y directivas de comportamiento para el agente.' },
  { title: 'Conexión de APIs', desc: 'Conectamos la API oficial de OpenAI con tus canales de mensajería (WhatsApp, Telegram o Web).' },
  { title: 'Ajuste Fino', desc: 'Evaluamos las respuestas del chatbot mediante pruebas piloto para asegurar precisión y coherencia.' }
];
...
const VALUE_POINTS = [
  { icon: 'fa-robot', label: 'Conexión WhatsApp' },
  { icon: 'fa-comments', label: 'Mesa de ayuda 24/7' },
  { icon: 'fa-brain', label: 'Semantic Query Analysis' },
  { icon: 'fa-code', label: 'OpenAI API Integration' }
];

const AutomatizacionDigital = () => {
    ...
    <div className="relative overflow-hidden min-h-screen bg-white dark:bg-[#0a0a0a] font-montserrat">
      ...
      <section className="relative overflow-hidden pt-20 pb-12" id="home">
        <div className="container mx-auto px-6 max-w-7xl relative z-10 flex flex-col items-center">
          ...
          <Breadcrumb
            paths={[{ label: 'Soluciones' }, { label: 'Integración de Sistemas' }]}
            badgeText="Integración de Sistemas — TECH HOME"
            icon="fa-robot"
            align="center"
          />
          ...
          <div className="flex flex-col xl:flex-row items-center xl:items-stretch gap-4 xl:-gap-8 justify-between w-full">
            ...
            <div className="w-full xl:w-[35%] flex flex-col gap-5 text-center items-center xl:justify-center animate-slide-down">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-[#a41e22] to-[#7f1d1d] text-white border border-[#c5a059]/40 shadow-md">
                ...
                <span className="text-[9px] font-black uppercase tracking-wider">Cuota mensual según uso + Mantenimiento</span>
              </div>
            </div>
            ...
            <div className="w-full xl:w-[52%] relative min-h-[280px] sm:min-h-[360px] md:min-h-[400px] lg:min-h-[440px] xl:min-h-[480px] flex items-center justify-center -mb-4 sm:mb-0">
              ...
              <div className="relative z-10 w-full max-w-[420px] sm:max-w-[540px] md:max-w-[620px] lg:max-w-[680px] xl:max-w-[740px] 2xl:max-w-[820px] ml-auto animate-fade-in-up delay-200 group/hero flex items-center justify-center mt-6 sm:mt-8">
                ...
                >
                  <img
                    src="/img/07_Servicios/soluciones/IntegracionDeSistemas.png"
                    alt="Procesos Automáticos"
                    loading="eager"
                    fetchPriority="high"
                    decoding="async"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-6 text-center flex flex-col items-center justify-center">
              ...
              <div className="space-y-6 text-[#5c6f8f] dark:text-slate-355 font-bold leading-relaxed text-sm sm:text-base max-w-2xl text-justify">
                <p>
                  Conectamos tus plataformas para que los datos fluyan y los procesos se automaticen sin intervención manual, unificando todas tus herramientas en un solo ecosistema.
                </p>
                <p>
                  <span className="font-black uppercase tracking-widest text-[#a41e22] dark:text-red-400">Incluye: </span>
                  APIs, Pasarelas de Pago, WhatsApp Business, Redes Sociales, Facturación Electrónica, CRM, ERP, Correos, Automatizaciones, Bases de Datos, Nube e Integraciones Personalizadas.
                </p>
              </div>
            </div>
              ...
              <div className="bg-[#1e1e1e] rounded-3xl shadow-2xl border border-neutral-800 overflow-hidden text-left relative group">
                ...
                <div className="bg-[#252526] p-3 flex items-center justify-between border-b border-neutral-900">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold ml-2 font-mono">openai_agent.js</span>
                  </div>
                ...
                <div className="p-5 font-mono text-[10.5px] sm:text-[11.5px] leading-relaxed text-slate-300 overflow-x-auto select-none space-y-1">
                  <div>const OpenAI = require("openai");</div>
                  <div>const openai = new OpenAI(&#123; apiKey: process.env.OPENAI_API_KEY &#125;);</div>
                  <div className="mt-2">async function chatWithAI(userQuery) &#123;</div>
                  <div className="pl-4">const completion = await openai.chat.completions.create(&#123;</div>
                  <div className="pl-8">model: "gpt-4-turbo",</div>
                  <div className="pl-8">messages: [</div>
                  <div className="pl-12">&#123; role: "system", content: "Eres un agente de ventas de TECH HOME..." &#125;,</div>
                  <div className="pl-12">&#123; role: "user", content: userQuery &#125;</div>
                  <div className="pl-8">]</div>
                  <div className="pl-4">&#125;);</div>
                  <div className="pl-4">return completion.choices[0].message.content;</div>
                  <div>&#125;</div>
                </div>
        ...
        <div className="w-full max-w-[1200px] xl:max-w-[1400px] mx-auto px-4 relative z-10 flex flex-col items-center">
          ...
          <div className="text-center mb-6 max-w-4xl mx-auto">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-[#111827] dark:text-white leading-[1.1] mb-6">
              ¿Cómo atiendes a tus prospectos hoy <br className="hidden md:block" />
            ...
            <p className="text-base sm:text-lg text-gray-500 dark:text-slate-400 font-medium leading-relaxed max-w-2xl mx-auto mb-6">
              Compara la atención manual lenta con respuestas genéricas frente a un agente de Procesos Automáticos que atiende 24/7 de forma personalizada.
            </p>
            ...
            <div className="flex justify-center">
              <div className="inline-flex p-2 bg-white dark:bg-[#0a0a0a] rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.08)] border border-gray-100 dark:border-[#a41e22]/20 relative z-50 ring-4 ring-gray-50 dark:ring-black/10 scale-[0.85] sm:scale-90 md:scale-100 origin-center mx-auto">
                ...
                >
                  <i className="fa-solid fa-triangle-exclamation"></i> Sin Agente de IA
                </button>
                <button
                ...
                >
                  <i className="fa-solid fa-circle-check"></i> Con Agente de IA
                </button>
              </div>
            </div>
          </div>
              ...
              <div className={`bg-white dark:bg-[#0a0a0a] rounded-[2.25rem] sm:rounded-[2.5rem] w-full h-full md:min-h-[560px] lg:min-h-[600px] relative overflow-hidden flex flex-col border border-red-50 dark:border-[#a41e22]/10 shadow-[0_20px_60px_-15px_rgba(239,68,68,0.15)] transition-all duration-500 pb-3 sm:pb-4 md:pb-5
                  ...
                  <div className="relative z-10 flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      <h3 className="text-xl sm:text-2xl font-black text-gray-800 dark:text-white leading-none">
                        Atención Manual de Mensajes
                      </h3>
                    </div>
                    <p className="text-center text-[11px] sm:text-sm text-red-500/80 font-semibold max-w-xl mx-auto">
                      Respuestas lentas con textos genéricos pregrabados y abandono de leads por la noche.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-2 sm:p-3 md:p-4 relative flex-1 flex flex-col rounded-[2rem] sm:rounded-[2.25rem] overflow-hidden">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 relative z-10 w-full h-full content-start">
                    {[
                      { title: "Respuestas con Copiar y Pegar", icon: "fa-keyboard", desc: "El operario digita lo mismo mil veces" },
                      { title: "Abandono de Noche", icon: "fa-moon", desc: "Clientes sin responder fuera de horario" },
                      { title: "Respuestas Genéricas", icon: "fa-comment-slash", desc: "Respuestas frías sin adaptabilidad" },
                      { title: "Clasificación Manual", icon: "fa-user-slash", desc: "El agente califica prospectos a mano" },
                      { title: "Pérdida de Mensajes", icon: "fa-envelope", desc: "Chats perdidos en la bandeja de entrada" },
                      { title: "Costos Operativos", icon: "fa-dollar-sign", desc: "Contratación de más personal para soporte" }
                    ].map((item, i) => (
                      <div
                        key={i}
            ...
            >
              <div className={`bg-gradient-to-br from-[#a41e22] to-[#7f1d1d] w-full h-full md:min-h-[560px] lg:min-h-[600px] relative overflow-hidden flex flex-col rounded-[2.25rem] sm:rounded-[2.5rem] shadow-2xl border border-white/20 transition-all duration-500 pb-3 sm:pb-4 md:pb-5
                  ...
                  <div className="relative z-10 flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      <h3 className="text-xl sm:text-2xl font-black text-white leading-none">
                        Agente de IA Tech Home
                      </h3>
                    </div>
                    <p className="text-center text-[11px] sm:text-sm text-blue-50/90 font-semibold max-w-xl mx-auto">
                      Entendido semántico avanzado de lenguaje, respuestas rápidas y registro automático.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-2 sm:p-3 md:p-4 relative flex-1 flex flex-col rounded-[2rem] sm:rounded-[2.25rem] overflow-hidden">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 relative z-10 w-full h-full content-start">
                    {[
                      { title: "Comprensión Semántica", icon: "fa-brain", desc: "Interpreta y responde según contexto" },
                      { title: "Atención Ininterrumpida 24/7", icon: "fa-clock", desc: "Responde dudas al instante a medianoche" },
                      { title: "Respuestas Únicas", icon: "fa-wand-magic-sparkles", desc: "Mensajes adaptados a cada cliente" },
                      { title: "Lead Scoring de Prospectos", icon: "fa-chart-line", desc: "Filtra e ingresa prospectos al CRM" },
                      { title: "Integración Completa", icon: "fa-link", desc: "Conexión directa con bases de datos" },
                      { title: "Eficiencia de Recursos", icon: "fa-circle-check", desc: "Libera a operarios de tareas mecánicas" }
                    ].map((item, i) => (
                      <div
                        key={i}
...
export default AutomatizacionDigital;
